...
export function getStorage(): Storage {
  ...
  // 检查是否在 Cloudflare Pages 环境中运行
  if (typeof process.env.CLOUDFLARE_KV !== 'undefined') {
    console.log('Storage System: Cloudflare KV namespace found.');
    try {
      const { CloudflareKVStorage } = require('./cloudflare-kv');
      storageInstance = new CloudflareKVStorage(process.env.CLOUDFLARE_KV);
      console.log('Storage System: CloudflareKVStorage initialized.');
      return storageInstance;
    } catch (error: any) {
      console.error('Storage System: Failed to initialize Cloudflare KV storage. Falling back.', error.message);
    }
  }
